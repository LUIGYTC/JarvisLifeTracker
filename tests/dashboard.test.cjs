...
test('dashboard renders safe labels, MXN, all charts and privacy reset without storage', () => {
  ...
  for (const label of ['Datos de demostración', 'Gastos por categoría', 'Ingresos vs gastos por día', 'Gastos por método', 'Movimientos recientes', '$10.00']) assert.ok(root.innerHTML.includes(label));
