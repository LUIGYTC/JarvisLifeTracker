...
export const DEBIT_RANGES = ["'TarjetasDebito'!A:A", "'TarjetasDebito'!C:C"];
const invalid = () => { throw new Error('Invalid debit data'); };
const empty = value => value == null || (typeof value === 'string' && !value.trim());

export function aggregateAvailableMoney(names = [], balances = []) {
  if (![names, balances].every(rows => Array.isArray(rows) && rows.length <= 10001 && rows.every(row => Array.isArray(row) && row.length <= 1))) invalid();
  if (!names.length && !balances.length) return { total: 0, cuentas: [] };
  if (names[0]?.[0] !== 'Tarjeta' || balances[0]?.[0] !== 'Saldo disponible') invalid();
  const cuentas = [];
  let total = 0;
  for (let i = 1; i < Math.max(names.length, balances.length); i++) {
    const nombre = names[i]?.[0], saldo = balances[i]?.[0];
    if (empty(nombre) && empty(saldo)) continue;
  ...
  return { total: total / 100, cuentas };
}
  ...
  return async () => {
    ...
    try {
      return await Promise.race([(async () => {
        ...
        if (!Array.isArray(data.valueRanges) || data.valueRanges.length !== 2) invalid();
        return aggregateAvailableMoney(data.valueRanges[0].values, data.valueRanges[1].values);
