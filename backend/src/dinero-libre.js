...
export const FREE_MONEY_RANGES = [...DEBIT_RANGES, "'Compromisos'!A:H", "'CortesTarjeta'!A:E", "'CortesTarjeta'!H:M", "'Movimientos'!A:H"];
...
export function calculateFreeMoney({ available, commitments = [], cutDates = [], cutAmounts = [], movements = [], today, anchor = '' }) {
  ...
  const expenses = rows(movements, ['Fecha', 'Hora', 'Tipo', 'Categoría', 'Monto', 'Descripción', 'Método', 'Destino'])
    .filter(row => !row.every(blank)).map(row => {
      const fecha = dateText(row[0]);
      if (!['Ingreso', 'Gasto', 'Transferencia', 'Pago tarjeta'].includes(row[2])) invalid();
      cents(row[4]);
      return { fecha, tipo: row[2], destino: row[2] === 'Pago tarjeta' ? label(row[7]) : '' };
    }).filter(row => row.tipo === 'Pago tarjeta' && row.fecha <= today);
  ...
  for (let i = 0; i < Math.max(dates.length, amounts.length); i++) {
    ...
    // Native Sheets timestamps contain a fractional day. Keep the date-only
    // conservative reconciliation rule, including payments on that same day.
    const actualizado = blank(amount[5]) ? null : dateText(typeof amount[5] === 'number' ? Math.floor(amount[5]) : amount[5]);
    ...
    if (expenses.some(movement => movement.fecha >= actualizado && movement.destino === nombre)) {
  ...
  return { estado: pendientes.length ? 'incompleto' : 'completo', periodo, dineroDisponible: disponible / 100,
    ...
    compromisos, compromisosInformativos, cortes, pendientes };
}
  ...
  return async () => {
    ...
    try {
      return await Promise.race([(async () => {
        ...
        const [names, balances, types, commitments, cutDates, cutAmounts, movements] = data.valueRanges.map(range => range.values === undefined ? [] : range.values);
        return calculateFreeMoney({ available: aggregateAvailableMoney(names, balances, types), commitments, cutDates, cutAmounts, movements, today: mexicoToday(now()), anchor });
