(() => {
  ...
  function render(root, state, data) {
    ...
    if (state !== 'loaded') {
      root.innerHTML = `<section class="shell"><div class="card" role="status">${state === 'loading' ? 'Cargando datos…' : state === 'demo' ? 'Exploración pública. Inicia sesión para cargar los datos de demostración.' : 'No pudimos cargar tus datos.'}</div></section>`;
      return;
    }
    validate(data);
    ...
    root.innerHTML = `<main class="shell financial-dashboard">
      <header class="top"><div><div class="brand">Tu actividad</div><p class="muted">Datos de demostración</p></div><span class="pill" role="status">${summary.movimientos ? 'Dashboard cargado' : 'Sin movimientos'}</span></header>
