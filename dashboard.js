(() => {
  ...
  function render(root, state, data) {
    ...
    root.innerHTML = `<main class="shell financial-dashboard">
      ...
      <section class="card section"><h2>Movimientos recientes</h2><div class="table-scroll">${recent.length ? `<table><thead><tr><th>Fecha</th><th>Descripción</th><th>Categoría</th><th>Método</th><th>Destino</th><th>Monto</th><th>Tipo</th></tr></thead><tbody>${recent.map(item => `<tr><td>${escape(item.fecha)}</td><td>${escape(item.descripcion)}</td><td>${escape(item.categoria)}</td><td>${escape(item.metodo)}</td><td>${escape(item.destino || "—")}</td><td>${amount(item.monto)}</td><td>${escape(item.tipo)}</td></tr>`).join('')}</tbody></table>` : '<p class="muted">Sin movimientos.</p>'}</div></section></main>`;
  }
