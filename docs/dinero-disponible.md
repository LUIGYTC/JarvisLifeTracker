...
Consulta exclusivamente `TarjetasDebito!A:A` (`Tarjeta`) y `TarjetasDebito!C:C`
(`Saldo disponible`) mediante una sola operación `values:batchGet`, con valores
evaluados sin formato. No consulta otras hojas ni devuelve banco, notas o fechas.

Respuesta: `{ total, cuentas: [{ nombre, saldo }] }`. La suma se realiza en
...
archivos estáticos, incluyendo el nuevo módulo, con versión `2.3.19`.
