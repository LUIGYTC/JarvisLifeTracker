(() => {
  ...
  const modules = [
    ...
    { id: 'rutinas', name: 'Rutinas', description: 'Hábitos, tareas y automatizaciones', icon: 'M3 6L5 8L8 4M11 6H21M3 14L5 16L8 12M11 14H21M11 21H21' },
  ...
  function mount(root, onExit) {
    ...
    root.innerHTML = `<nav class="dashboard-nav shell" aria-label="Navegación de Jarvis">
      ...
      <main id="jarvis-home" class="shell jarvis-home" tabindex="-1">
        ...
        <div id="dashboard-data" tabindex="-1"></div></section>`;
    ...
    function show(view) {
      if (!active || !['home', 'finanzas'].includes(view)) return;
      current = view;
      home.hidden = view !== 'home';
      finance.hidden = view !== 'finanzas';
      window.scrollTo(0, 0);
      (view === 'home' ? home : content).focus({ preventScroll: true });
    }
